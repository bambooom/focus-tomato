...
class Timer extends EventEmitter {
  ...
  pause() {
    ...
    const periodElapsed = (Date.now() - this.checkpointStartAt) / 1000;
...
class PomodoroTimer extends EventEmitter {
  ...
  advanceTimer: boolean;
  pomodoros: number;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  settings: Record<string, any>;
  timer: Timer;
  private _phase: EnumOption;
  ...
  _updateTimer() {
    const { duration } = {
      [Phase.Focus.toString()]: this.settings.focus,
      [Phase.ShortBreak.toString()]: this.settings.shortBreak,
      [Phase.LongBreak.toString()]: this.settings.longBreak
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    }[this.phase as unknown as string] as Record<string, any>;
  ...
  get pomodorosUntilLongBreak() {
    const { interval } = this.settings.longBreak;
    if (!interval) {
      return null;
    }
